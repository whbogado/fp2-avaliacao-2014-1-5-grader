...
public class ClassScannerStepdefs {
    ...
    public void classExtends(String clsVar, String superClassName) throws Throwable {
        Class clazz = (Class) bsh.get(clsVar);
        ...
        assertTrue(superClass.isAssignableFrom(clazz),
            String.format("'%s' is not assignable from '%s'", clazz.getName(), superClassName));
        Reporter.log(String.format("'%s' is assignable from '%s'", clazz.getName(), superClassName), true);        
    }
    ...
    @Given("I set input from stream <(.+)>")
    public void setInputFromStream(String isVar) throws Throwable {
        if (isVar.equals("default")) {
            System.setIn(SYSTEM_IN);
            bsh.unset("in");
        } else {
            InputStream is = (InputStream) bsh.get(isVar);
            System.setIn(is);
            bsh.set("in", is);
        }
    }
